...
describe('EmotionalStateFragranceController', () => {
  ...
  describe('update', () => {
    it('should call service.update and return the result', async () => {
      const dto: UpdateEmotionalStateFragranceDto = { fragranceId: 'uuid-new-fragrance' };
      const mockResult = { id: 'uuid', fragrance: {}, emotionalState: {} };
  
      mockService.update.mockResolvedValue(mockResult);
  
      const result = await controller.update('uuid-emotional-state', 'uuid-fragrance', dto);
  
      expect(result).toEqual(mockResult);
      expect(mockService.update).toHaveBeenCalledWith('uuid-emotional-state', 'uuid-fragrance', dto);
    });
  });
});