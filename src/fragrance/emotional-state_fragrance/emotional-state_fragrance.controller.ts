...
export class EmotionalStateFragranceController {
  ...
  @Patch(':emotionalStateId/:fragranceId')
  @Roles('admin')
  update(
    @Param('emotionalStateId') emotionalStateId: string,
    @Param('fragranceId') fragranceId: string,
    @Body() updateEmotionalStateFragranceDto: UpdateEmotionalStateFragranceDto,
  ) {
    return this.emotionalStateFragranceService.update(
      emotionalStateId,
      fragranceId,
