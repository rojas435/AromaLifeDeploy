...
export class CustomCandleService {
  constructor(
    ...
    private readonly userRepository: Repository<User>, // Repositorio para buscar usuarios
  ) {}

  async create(createCustomCandleDto: CreateCustomCandleDto) {
    const { containerId, fragranceId, emotionalStateId, userId, ...candleData } = createCustomCandleDto;
  ...
  async update(id: string, updateCustomCandleDto: UpdateCustomCandleDto) {
    const { containerId, fragranceId, emotionalStateId, ...candleData } = updateCustomCandleDto;
